...
public class LoginController extends BaseController {
    ...
    @ResponseBody
    public Map<String, Object> login(@RequestParam(value = "captcha") String captcha, Merchant merchant, HttpServletRequest request) {
        Map<String, Object> resultMap = getResultMap();

        HttpSession session = request.getSession();
        String realCaptcha = (String) session.getAttribute(AppConstants.KEY_CAPTCHA);
        log.info("session中的验证码为：{}", realCaptcha);
        log.info("商户上送的验证码为：{}", captcha);
        ...
        UsernamePasswordToken token = new UsernamePasswordToken(merchant.getMerchCo(), merchant.getPassword());
        ...
        try {
            ...
            log.warn("未知商户号", uae);
            setResultMapFailure(resultMap, "未知商户号");
            return resultMap;
        } catch (IncorrectCredentialsException ice) {
            log.warn("商户号或密码错误", ice);
            setResultMapFailure(resultMap, "商户号或密码错误");
            return resultMap;
